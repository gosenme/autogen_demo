...
nvda = yf.download('NVDA', start='2023-01-01', end=datetime.today().strftime('%Y-%m-%d'))
tsla = yf.download('TSLA', start='2023-01-01', end=datetime.today().strftime('%Y-%m-%d'))
...
plt.plot(nvda['Close'], label='NVDA YTD Change')
plt.plot(tsla['Close'], label='TSLA YTD Change')
